...
// Generic function to fetch an account
...
{
    log::debug!("Fetching account '{}'", account_number);
    ...
    if account.is_some() {
        log::debug!("Account '{}' fetched successfully", account_number);
    } else {
        log::warn!("Account '{}' not found", account_number);
    }
